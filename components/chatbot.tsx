...
/** Swaroop Bhai mascot — bearded mountain buddy */
const MASCOT_EMOJI = "\u{1F9D4}\u{1F3FB}\u{200D}\u{2642}\u{FE0F}";
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-xl shadow-emerald-500/30 transition-shadow hover:shadow-emerald-500/50"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 flex w-[380px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40 backdrop-blur-xl"
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-white/10 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 px-4 py-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-xl">
                <span aria-hidden>{MASCOT_EMOJI}</span>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin">
              ...
              {!hasMessages && (
                <div className="flex flex-col items-center text-center py-6">
                  <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border border-emerald-500/20 text-3xl">
                    <span aria-hidden>{MASCOT_EMOJI}</span>
                  </div>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  {msg.role === "assistant" && (
                    <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 text-sm">
                      <span aria-hidden>{MASCOT_EMOJI}</span>
                    </div>
                  )}
